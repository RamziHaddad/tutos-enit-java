...
class CompteursWithMockedLecteurTest {
	...
	@Test
	void givenAReaderAndNonEmptyList_whenCountingCompteur4_thenAccessesList() {
		Compteur4 compteur = spy(Compteur4.class);
		List<String> mockedMots = spy(new ArrayList<String>(mots));
		Map<String, Long> comptage = compteur.occurrencesParMot(mockedMots);
		verify(mockedMots,times(mots.size()+comptage.size()-1)).get(anyInt());
